...
function getCharacters() {
  const char_url = `http://gateway.marvel.com/v1/public/characters?&ts=1&apikey=6d47a63ab8ed920ce1fcc082d0ac903f&hash=e02da18fd84011b0c9b6ff78ade4ec2f&limit=100`;
  getCharApi(char_url);
}
getCharacters();
...
function searchListFunc(data) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    li.innerHTML = `
                                <a href=${
                                  "/SUPER-HERO/character.html?character=" +
                                  data[i].id
                                }git class="flex j-sb a-c" style="text-decoration:none;color:black;">
                                    <div class="left flex a-c">
                                        <img src="${
                                          data[i].thumbnail.path
                                        }.jpg" alt="" id="img1">
      ...
      data[i].id
    }"><i class="fa-solid fa-plus addFavi"></i></button>
                            `;
